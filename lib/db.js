// Camada de banco de dados: usa Vercel KV se disponível, senão usa Map em memória
...
async function initDefaultUsers() {
  ...
  const defaults = [
    { id: 'usr_system', username: 'system', password: bcrypt.hashSync('system@2026', 10), nome: 'System Master', role: 'SYSTEM', permissions: null, active: true },
    { id: 'usr_admin',  username: 'admin',  password: bcrypt.hashSync('admin@2026', 10),  nome: 'Administrador', role: 'ADMIN',   permissions: null, active: true },
  ];
...
export async function getCadastros(tipo) {
  ...
  return items.filter(Boolean).sort((a, b) => (a.nome || a.id || '').localeCompare(b.nome || b.id || ''));
}
